...
public class UseTimeAop {
	...
	@Around("product() || folder() || naver()") // 실행할 지점 선택
	public Object execute(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			// 핵심기능 수행
			Object output = joinPoint.proceed(); // 로직이 실행됨
			return output;
		} finally {
			// 측정 종료 시간
			...
			if (auth != null && auth.getPrincipal().getClass() == UserDetailsImpl.class) {
				...
				log.info(
					"[API Use Time] Username: " + loginUser.getUsername() + ", Total Time: " + apiUseTime.getTotalTime()
						+ " ms");
